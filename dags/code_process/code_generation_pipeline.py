...
from llm_code_utils import check_python_solution

# --- File Constants ---
CORRECT_FILE = "correct_code_results.jsonl"
FAILED_FILE = "failed_code_results.jsonl"

def load_jsonl_with_escape_fix(file_path):
    """
    Loads a JSONL file, attempting to fix common 'Invalid \escape' errors.
    (Keeping this function from the reference, though less common for general code
    problems, it maintains the structural integrity of the original pipeline.)
    """
    questions = []
...
def process_code_problems(input_filepath: str):
    """
    Main pipeline to process questions, generate Python code and test, execute it,
    and save results to separate JSONL files.
    """
    ...
    executor = PythonExecutorBridge()

    # Clear previous results
    for f in [CORRECT_FILE, FAILED_FILE]:
        if os.path.exists(f):
            os.remove(f)
    ...
    for i, question_data in enumerate(questions):
        ...
        # Pass the entire data dictionary, as the 'question' key is within it
        result = check_python_solution(executor, question_data) 
...
if __name__ == "__main__":
    # NOTE: Assuming an 'input_questions.jsonl' file exists with questions
    input_file_path = 'input_questions.jsonl' 
    process_code_problems(input_file_path)